...
function productSessionFromRow(row) {
  const technicalOwner = row.technical_owner_id
    ? {
        type: row.technical_owner_type,
        id: row.technical_owner_id,
        name: row.technical_owner_name,
        avatarUrl: row.technical_owner_avatar_url,
      }
    : null;
  return {
    id: row.id,
    ...
    title: row.title,
    status: row.status,
    productDocument: row.product_document,
    documentVersion: row.document_version,
    approvedDocument: row.approved_document,
    approvedVersion: row.approved_version,
    approvedAt: row.approved_at,
    approvedBy: row.approved_by,
    technicalTaskId: row.technical_task_id,
    technicalAiThreadId: row.technical_ai_thread_id,
    technicalDocument: row.technical_document,
    technicalDocumentVersion: row.technical_document_version,
    technicalApprovedDocument: row.technical_approved_document,
    technicalApprovedVersion: row.technical_approved_version,
    technicalApprovedAt: row.technical_approved_at,
    technicalApprovedBy: row.technical_approved_by,
    technicalOwner,
    deliveryNote: row.delivery_note,
    deliverySubmittedAt: row.delivery_submitted_at,
    deliverySubmittedBy: row.delivery_submitted_by,
    implementationPr: row.implementation_pr,
    testDeploymentUrl: row.test_deployment_url,
    testDeploymentWorkflowRun: row.test_deployment_workflow_run,
    testDeploymentImmutableTag: row.test_deployment_immutable_tag,
    testDeploymentPrNumbers: JSON.parse(row.test_deployment_pr_numbers ?? "[]"),
    acceptanceStatus: row.acceptance_status,
    acceptanceNote: row.acceptance_note,
    acceptanceAt: row.acceptance_at,
    acceptanceBy: row.acceptance_by,
    creator: {
...
export class TaskboardDatabase {
  ...
  #migrate() {
    this.database.exec(`
      ...
      CREATE TABLE IF NOT EXISTS product_sessions (
        id TEXT PRIMARY KEY,
        project_id TEXT NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
        ai_thread_id TEXT NOT NULL UNIQUE REFERENCES ai_chat_threads(id) ON DELETE CASCADE,
        title TEXT NOT NULL,
        status TEXT NOT NULL CHECK (status IN (
          'discovery', 'draft', 'approved', 'handed_off'
        )),
        product_document TEXT NOT NULL DEFAULT '',
        document_version INTEGER NOT NULL DEFAULT 0 CHECK (document_version >= 0),
        approved_document TEXT,
        approved_version INTEGER,
        approved_at TEXT,
        approved_by TEXT,
        technical_task_id TEXT REFERENCES tasks(id),
        technical_ai_thread_id TEXT REFERENCES ai_chat_threads(id) ON DELETE SET NULL,
        technical_document TEXT NOT NULL DEFAULT '',
        technical_document_version INTEGER NOT NULL DEFAULT 0 CHECK (technical_document_version >= 0),
        technical_approved_document TEXT,
        technical_approved_version INTEGER,
        technical_approved_at TEXT,
        technical_approved_by TEXT,
        technical_owner_type TEXT CHECK (technical_owner_type IN ('user', 'agent')),
        technical_owner_id TEXT,
        technical_owner_name TEXT,
        technical_owner_avatar_url TEXT,
        delivery_note TEXT NOT NULL DEFAULT '',
        delivery_submitted_at TEXT,
        delivery_submitted_by TEXT,
        implementation_pr TEXT,
        test_deployment_url TEXT,
        test_deployment_workflow_run TEXT,
        test_deployment_immutable_tag TEXT,
        test_deployment_pr_numbers TEXT NOT NULL DEFAULT '[]',
        acceptance_status TEXT CHECK (acceptance_status IN ('pending', 'accepted', 'changes_requested')),
    ...
    const productSessionMigrations = [
      ["technical_ai_thread_id", "TEXT REFERENCES ai_chat_threads(id) ON DELETE SET NULL"],
      ["technical_document", "TEXT NOT NULL DEFAULT ''"],
      ["technical_document_version", "INTEGER NOT NULL DEFAULT 0"],
      ["technical_approved_document", "TEXT"],
      ["technical_approved_version", "INTEGER"],
      ["technical_approved_at", "TEXT"],
      ["technical_approved_by", "TEXT"],
    ...
    for (const [column, definition] of productSessionMigrations) {
      if (!productSessionColumns.some((candidate) => candidate.name === column)) {
        this.database.exec(`ALTER TABLE product_sessions ADD COLUMN ${column} ${definition}`);
      }
    }
    this.database.exec(`
      CREATE UNIQUE INDEX IF NOT EXISTS product_sessions_technical_ai_thread
      ON product_sessions(technical_ai_thread_id)
      WHERE technical_ai_thread_id IS NOT NULL
    `);
  ...
  updateAiChatThread(id, changes) {
    ...
    const columns = {
      ...
      sandbox: "sandbox",
      workspacePath: "origin_workspace_path",
    };
    const assignments = [];
    const values = [];
  ...
  listProductSessions(projectId, { page, pageSize, query = "", status = null }) {
    ...
    const totalItems = this.database.prepare(`
      SELECT COUNT(*) AS count FROM product_sessions WHERE ${where}
    `).get(...values).count;
    const data = this.database.prepare(`
      ...
      ORDER BY updated_at DESC, id
      LIMIT ? OFFSET ?
    `).all(...values, pageSize, (page - 1) * pageSize).map(productSessionFromRow);
    return {
      data,
      pagination: {
        page,
  ...
  getProductSession(id) {
    const row = this.database.prepare("SELECT * FROM product_sessions WHERE id = ?").get(id);
    return row ? productSessionFromRow(row) : null;
  }

  getProductSessionByAiThreadId(aiThreadId) {
    const row = this.database.prepare(
      "SELECT * FROM product_sessions WHERE ai_thread_id = ?",
    ).get(aiThreadId);
    return row ? productSessionFromRow(row) : null;
  }

  getProductSessionByTechnicalAiThreadId(aiThreadId) {
    const row = this.database.prepare(
      "SELECT * FROM product_sessions WHERE technical_ai_thread_id = ?",
    ).get(aiThreadId);
    return row ? productSessionFromRow(row) : null;
  }
  ...
  createProductSession(input) {
    ...
    this.database.prepare(`
      INSERT INTO product_sessions (
        id, project_id, ai_thread_id, title, status,
        product_document, document_version,
        creator_type, creator_id, creator_name, creator_avatar_url,
        created_at, updated_at
      ) VALUES (?, ?, ?, ?, 'discovery', '', 0, ?, ?, ?, ?, ?, ?)
    `).run(
  ...
  updateProductSessionDocument(id, expectedVersion, content) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    if (current.status === "approved" || current.status === "handed_off") {
      throw new ApiError(409, "PRODUCT_SESSION_APPROVED", "Approved product documents cannot be edited");
    }
    if (current.documentVersion !== expectedVersion) {
      throw new ApiError(409, "VERSION_CONFLICT", "Product document changed since it was last read", {
        expectedVersion,
        actualVersion: current.documentVersion,
      });
    }
    const result = this.database.prepare(`
      UPDATE product_sessions
      SET product_document = ?, document_version = document_version + 1,
          status = 'draft', updated_at = ?
      WHERE id = ? AND document_version = ?
    `).run(content, now(), id, expectedVersion);
    if (result.changes !== 1) {
      const latest = this.getProductSession(id);
      throw new ApiError(409, "VERSION_CONFLICT", "Product document changed since it was last read", {
        expectedVersion,
        actualVersion: latest?.documentVersion,
      });
    }
    return this.getProductSession(id);
  }

  approveProductSession(id, actor) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    if (!current.productDocument.trim()) {
      throw new ApiError(422, "PRODUCT_DOCUMENT_REQUIRED", "Save a product document before approval");
    }
    if (current.status === "handed_off") return current;
    const timestamp = now();
    this.database.prepare(`
      UPDATE product_sessions
      SET status = 'approved', approved_document = product_document,
          approved_version = document_version, approved_at = ?, approved_by = ?, updated_at = ?
      WHERE id = ?
    `).run(timestamp, actor.name, timestamp, id);
    return this.getProductSession(id);
  }

  handoffProductSession(id, taskId) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    if (current.status !== "approved" && current.status !== "handed_off") {
      throw new ApiError(409, "PRODUCT_SESSION_NOT_APPROVED", "Approve the product document before handoff");
    }
    if (current.technicalTaskId) return current;
    this.database.prepare(`
      UPDATE product_sessions
      SET status = 'handed_off', technical_task_id = ?, updated_at = ?
      WHERE id = ? AND technical_task_id IS NULL
    `).run(taskId, now(), id);
    return this.getProductSession(id);
  }

  attachTechnicalAiThread(id, aiThreadId) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    if (current.status !== "handed_off") {
      throw new ApiError(409, "PRODUCT_SESSION_NOT_HANDED_OFF", "Hand off the product specification before starting technical design");
    }
    if (current.technicalAiThreadId) return current;
    const timestamp = now();
    const result = this.database.prepare(`
      UPDATE product_sessions
      SET technical_ai_thread_id = ?, updated_at = ?
      WHERE id = ? AND technical_ai_thread_id IS NULL
    `).run(
      aiThreadId,
      timestamp,
      id,
    );
    if (result.changes !== 1) return this.getProductSession(id);
    return this.getProductSession(id);
  }

  claimTechnicalOwner(id, actor) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    if (current.technicalOwner?.id === actor.id) return current;
    this.database.prepare(`
      UPDATE product_sessions
      SET technical_owner_type = ?, technical_owner_id = ?,
          technical_owner_name = ?, technical_owner_avatar_url = ?, updated_at = ?
      WHERE id = ?
    `).run(actor.type, actor.id, actor.name, actor.avatarUrl, now(), id);
    return this.getProductSession(id);
  }

  updateTechnicalDocument(id, expectedVersion, content, actor) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    if (current.status !== "handed_off") {
      throw new ApiError(409, "PRODUCT_SESSION_NOT_HANDED_OFF", "Technical design starts after product handoff");
    }
    if (current.technicalApprovedDocument !== null) {
      throw new ApiError(409, "TECHNICAL_DOCUMENT_APPROVED", "Approved technical documents cannot be edited");
    }
    if (current.technicalDocumentVersion !== expectedVersion) {
      throw new ApiError(409, "VERSION_CONFLICT", "Technical document changed since it was last read", {
        expectedVersion,
        actualVersion: current.technicalDocumentVersion,
      });
    }
    const timestamp = now();
    const result = this.database.prepare(`
      UPDATE product_sessions
      SET technical_document = ?,
          technical_document_version = technical_document_version + 1,
          technical_owner_type = ?, technical_owner_id = ?,
          technical_owner_name = ?, technical_owner_avatar_url = ?,
          updated_at = ?
      WHERE id = ? AND technical_document_version = ?
    `).run(
      content,
      actor.type,
      actor.id,
    ...
    if (result.changes !== 1) {
      ...
      throw new ApiError(409, "VERSION_CONFLICT", "Technical document changed since it was last read", {
        expectedVersion,
        actualVersion: latest?.technicalDocumentVersion,
      });
    }
    return this.getProductSession(id);
  }

  approveTechnicalDocument(id, actor) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    ...
    if (!current.technicalDocument.trim()) {
      throw new ApiError(422, "TECHNICAL_DOCUMENT_REQUIRED", "Save a technical document before approval");
    }
    if (current.technicalApprovedDocument !== null) return current;
    const timestamp = now();
    this.database.prepare(`
      UPDATE product_sessions
      SET technical_approved_document = technical_document,
          technical_approved_version = technical_document_version,
          technical_approved_at = ?, technical_approved_by = ?,
          technical_owner_type = ?, technical_owner_id = ?,
          technical_owner_name = ?, technical_owner_avatar_url = ?,
          updated_at = ?
      WHERE id = ?
  ...
  submitForProductAcceptance(id, input, actor) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    if (current.technicalApprovedDocument === null) {
      throw new ApiError(409, "TECHNICAL_DOCUMENT_NOT_APPROVED", "Approve the technical document before product acceptance");
    }
    const timestamp = now();
    this.database.prepare(`
      UPDATE product_sessions
      SET delivery_note = ?, delivery_submitted_at = ?, delivery_submitted_by = ?,
          implementation_pr = ?, test_deployment_url = ?,
          test_deployment_workflow_run = ?, test_deployment_immutable_tag = ?,
          test_deployment_pr_numbers = ?,
          acceptance_status = 'pending', acceptance_note = NULL,
          acceptance_at = NULL, acceptance_by = NULL, updated_at = ?
      WHERE id = ?
    `).run(
      input.note,
      timestamp,
      ...
      input.testDeployment.url,
      input.testDeployment.workflowRun,
      input.testDeployment.immutableTag,
      JSON.stringify(input.testDeployment.prNumbers),
      timestamp,
      id,
    );
    return this.getProductSession(id);
  ...
  recordProductAcceptance(id, outcome, note, actor) {
    const current = this.getProductSession(id);
    if (!current) {
      throw new ApiError(404, "PRODUCT_SESSION_NOT_FOUND", `Product session '${id}' does not exist`);
    }
    if (!current.deliverySubmittedAt) {
