...
type BoardView = "readme" | "dashboard" | "product" | "issues" | "list" | "gantt";
...
const ProductCollaborationView = lazy(() => import("./components/ProductCollaborationView").then((module) => ({
  default: module.ProductCollaborationView,
})));
...
function readProjectBoardView(projectId: string): BoardView {
  ...
  return view === "readme" || view === "dashboard" || view === "product" || view === "list" || view === "gantt" || view === "issues"
    ? view
    : "issues";
}
...
export function App() {
  ...
  const productCollaborationAvailable = taskboardMetadata?.capabilities?.productCollaboration === true;
  const publicAccessMode = taskboardMetadata?.capabilities?.publicAccess === true;
  const publicProductMode = publicAccessMode
    && taskboardMetadata?.capabilities?.publicRole === "product";
  const productCollaborationWrite = taskboardMetadata?.capabilities?.productCollaborationWrite !== false;
  const technicalCollaborationWrite = taskboardMetadata?.capabilities?.technicalCollaborationWrite !== false;
  const taskWrite = taskboardMetadata?.capabilities?.taskWrite !== false;
  ...
  const currentUser = taskboardMetadata?.currentActor ?? hostContext?.user ?? {
    ...DEFAULT_USER_ACTOR,
  ...
  useEffect(() => {
    if (selectedProjectId) {
      setBoardView(publicProductMode && selectedProjectId !== ALL_PROJECTS_ID
        ? "product"
        : selectedProjectId === ALL_PROJECTS_ID
          ? "issues"
          : readProjectBoardView(selectedProjectId));
    }
  }, [publicProductMode, selectedProjectId]);

  useEffect(() => {
    if (!publicProductMode || selectedProjectId !== ALL_PROJECTS_ID) return;
    const firstProductProject = projects.find((project) => (
      project.id !== GLOBAL_PROJECT_ID && project.source !== "jira"
    ));
    if (firstProductProject) changeProject(firstProductProject.id);
  }, [projects, publicProductMode, selectedProjectId]);
  ...
  const loadProjectList = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const isPublicAccess = metadata.capabilities?.publicAccess === true;
      const [workspaces, nextJiraConnection, nextTemporaryTasks] = await Promise.all([
        isPublicAccess
          ? Promise.resolve({} as Record<string, string>)
          : listDeviceWorkspaces(signal),
        isPublicAccess ? Promise.resolve(null) : getJiraConnection(signal),
      ...
      setTaskboardMetadata((current) => (
        ...
        && JSON.stringify(current.capabilities) === JSON.stringify(metadata.capabilities)
        && JSON.stringify(current.currentActor) === JSON.stringify(metadata.currentActor)
      ...
      if (metadata.currentActor) setCurrentUserActor(metadata.currentActor);
  ...
  useEffect(() => {
    if (!taskboardMetadata || publicAccessMode) {
      ...
      <main className="workspace">
        <header className="workspace-header">
            ...
            <div className="workspace-kicker">
              ...
              <div className="header-project-switcher" data-project-switcher>
                <button
                  ...
                  type="button"
                  aria-label={publicProductMode
                    ? text("当前项目", "Current project")
                    : text("切换项目", "Switch project")}
                  aria-haspopup={publicProductMode ? undefined : "menu"}
                  aria-expanded={publicProductMode ? undefined : projectMenuOpen}
                  tabIndex={publicProductMode ? -1 : undefined}
                  onClick={publicProductMode ? undefined : () => {
                    setProjectContextMenu(null);
                ...
                >
                  ...
                  {!publicProductMode && <TaskboardIcon className="project-switcher-chevron" name="dropdown" />}
                </button>
                {!publicProductMode && projectMenuOpen && (
                  <div className="header-project-menu" role="menu" aria-label={text("项目", "Projects")}>
                    ...
                    {!publicAccessMode && <div className="project-menu-actions">
                      <div className="project-menu-divider" role="separator" />
                      <button
                        type="button"
                        role="menuitem"
          ...
          <div className="header-actions">
            {selectedProject && !publicAccessMode && (
            ...
            {selectedProjectId && !isJiraProject && taskWrite && (
              <button
          ...
          <div className="view-tabs" aria-label={text("看板视图", "Board views")}>
            ...
            {!isAllProjects && !isJiraProject && productCollaborationAvailable && (
              <button
                ...
                type="button"
                aria-pressed={boardView === "product"}
                onClick={() => selectBoardView("product")}
              >
        ...
        {detailTask && selectedProject ? (
          ...
          />
        ) : boardView === "product" && selectedProject ? (
          <Suspense fallback={<div className="board-view-loading">{text("正在打开产品协作…", "Opening product collaboration…")}</div>}>
            <ProductCollaborationView
              projectId={selectedProject.id}
              canWrite={productCollaborationWrite}
              canTechnicalWrite={technicalCollaborationWrite}
              tasks={tasks.filter((task) => task.projectId === selectedProject.id)}
              onTaskCreated={(task) => {
                setTasks((current) => [task, ...current.filter((item) => item.id !== task.id)]);
                setAnnouncement(text("技术任务已创建", "Technical issue created"));
              }}
              onOpenTask={(taskId) => {
                const task = tasksRef.current.find((item) => item.id === taskId);
                if (!task) {
                  setActionError(text("技术任务尚未加载，请刷新后重试", "Technical issue is not loaded yet. Refresh and try again."));
                  return;
                }
                openTaskDetail(task);
              }}
