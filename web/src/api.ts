...
export async function listProductSessions(
  ...
  options: {
    ...
    pageSize?: number;
    query?: string;
    status?: ProductSession["status"] | null;
  } = {},
  signal?: AbortSignal,
): Promise<PaginatedResult<ProductSession>> {
  const query = new URLSearchParams({
    projectId,
  ...
  return request<PaginatedResult<ProductSession>>(`/api/product-sessions?${query}`, { signal });
}
...
export async function getProductSession(
  sessionId: string,
  signal?: AbortSignal,
): Promise<ProductSessionSnapshot> {
  return request<ProductSessionSnapshot>(
...
export async function saveProductDocument(
  ...
  const data = await request<{ session: ProductSession }>(
    `/api/product-sessions/${encodeURIComponent(sessionId)}/document`,
...
export async function saveTechnicalDocument(
  ...
  const data = await request<{ session: ProductSession }>(
    `/api/product-sessions/${encodeURIComponent(sessionId)}/technical-document`,
    { method: "PUT", body: JSON.stringify({ content, version }) },
  );
  return data.session;
}

export async function approveTechnicalDocument(
  sessionId: string,
): Promise<{ session: ProductSession; task: Task }> {
  return request<{ session: ProductSession; task: Task }>(
    `/api/product-sessions/${encodeURIComponent(sessionId)}/technical-approve`,
    { method: "POST" },
  );
}

export async function submitProductAcceptanceReview(
  sessionId: string,
  input: {
    note: string;
...
export async function recordProductAcceptance(
  sessionId: string,
  outcome: "accepted" | "changes_requested",
  note: string,
): Promise<{ session: ProductSession; task: Task }> {
  return request<{ session: ProductSession; task: Task }>(
    `/api/product-sessions/${encodeURIComponent(sessionId)}/acceptance`,
    { method: "POST", body: JSON.stringify({ outcome, note }) },
  );
}

export async function approveProductSession(
  sessionId: string,
): Promise<{ session: ProductSession; task: Task }> {
  return request<{ session: ProductSession; task: Task }>(
    `/api/product-sessions/${encodeURIComponent(sessionId)}/approve`,
    { method: "POST" },
  );
}

export function subscribeProductSession(
  sessionId: string,
  onHint: () => void,
  onError?: () => void,
): () => void {
  const source = new EventSource(
    resolveTaskboardUrl(`/api/product-sessions/${encodeURIComponent(sessionId)}/events`),
  );
  source.addEventListener("product.session", onHint);
  if (onError) source.addEventListener("error", onError);
  return () => source.close();
}
