...
test("each device stores an independent workspace path for every project", () => {
  ...
  assert.match(appSource, /const \[workspaces, nextJiraConnection, nextTemporaryTasks\] = await Promise\.all\(\[/);
  assert.match(appSource, /isPublicAccess[\s\S]*?Promise\.resolve\(\{\}[\s\S]*?: listDeviceWorkspaces\(signal\)/);
...
test("the project header exposes project, automation, and create controls", () => {
  assert.match(appSource, /className="header-project-button"[\s\S]*?aria-haspopup=\{publicProductMode \? undefined : "menu"\}/);
  assert.match(appSource, /!publicProductMode && projectMenuOpen && \([\s\S]*?className="header-project-menu" role="menu" aria-label=\{text\("项目", "Projects"\)\}/);
