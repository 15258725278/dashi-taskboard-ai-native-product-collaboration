...
import { AiChatService, loadProductAgentConfigArgs } from "../server/ai-chat.mjs";
import {
  ComposerCatalog,
...
test("product Agent config keeps only non-secret model provider connection fields", async () => {
  ...
  try {
    await writeFile(path.join(directory, "config.toml"), [
      ...
      '[model_providers.PrivateProvider.http_headers]',
...
test("product and technical collaboration turns use the restricted permission profile", async () => {
  const fixture = await createFixture();
  try {
    const thread = await fixture.service.createThread({
      projectId: "project",
    ...
    fixture.database.createProductSession({
      projectId: "project",
    ...
    const run = await fixture.service.startTurn(thread.id, { message: "clarify the request" });
    ...
    assert.deepEqual(capture.args, [
      ...
      "--disable", "goals",
      "--disable", "hooks",
      "-c", 'default_permissions="taskboard-product-agent"',
      "-c", 'permissions.taskboard-product-agent={workspace_roots={"."=true},filesystem={":minimal"="read",":workspace_roots"="read"}}',
    ...
    assert.equal(capture.prompt.includes("manage-taskboard"), false);
    assert.match(capture.prompt, /clarify the request/);

    const technicalThread = await fixture.service.createThread({
      projectId: "project",
      sandbox: "read-only",
    });
    const originalLookup = fixture.database.getProductSessionByTechnicalAiThreadId.bind(fixture.database);
    fixture.database.getProductSessionByTechnicalAiThreadId = (threadId) => (
      threadId === technicalThread.id ? { id: "technical-session" } : originalLookup(threadId)
    );
    const technicalRun = await fixture.service.startTurn(technicalThread.id, {
      message: "draft the technical plan",
    });
    await waitFor(() => fixture.service.getRun(technicalRun.id)?.status !== "running");
    const captures = (await readFile(fixture.capturePath, "utf8"))
      .trim()
      .split("\n")
      .map(JSON.parse);
    const technicalCapture = captures.at(-1);
    assert.equal(technicalCapture.args.includes("--ignore-user-config"), true);
    assert.equal(technicalCapture.args.includes("--add-dir"), false);
    assert.equal(technicalCapture.prompt.includes("manage-taskboard"), false);
    assert.match(technicalCapture.prompt, /draft the technical plan/);
  } finally {
    await fixture.close();
  }
});
